#!/bin/csh -f
## For c6a.48xlarge (192 vcpu - 96 cpu)
...
#SBATCH --exclusive
#SBATCH -J CMAQ
#SBATCH -o /shared/build/openmpi_gcc/CMAQ_v533/CCTM/scripts/run_cctmv5.3.3_Bench_2016_12US2.c6a.48xlarge.96.12x8pe.2day.pcluster.pin.log
#SBATCH -e /shared/build/openmpi_gcc/CMAQ_v533/CCTM/scripts/run_cctmv5.3.3_Bench_2016_12US2.c6a.48xlarge.96.12x8pe.2day.pcluster.pin.log
...
#> Set General Parameters for Configuring the Simulation
 ...
 set APPL      = 2016_CONUS_c6a.48xlarge_96_12x8pe_pin        #> Application Name (e.g. Gridname)
...
#        else if ( $SLURM_NTASKS_PER_NODE == "96" ) then
...
#        else if ( $SLURM_NTASKS_PER_NODE == "64" ) then

#        setenv PIN_PROCESSOR_LIST "--bind-to cpulist:ordered --cpu-set 0,1,2,3,8,9,10,11,16,17,18,19,24,25,26,27,30,31,32,33,38,39,40,41,46,47,48,49,54,55,56,57,60,61,62,63,68,69,70,71,76,77,78,79,84,85,86,87,90,91,92,93,98,99,100,101,106,107,108,109,114,115,116,117 --report-bindings"

#        else if ( $SLURM_NTASKS_PER_NODE == "32" ) then

#        setenv PIN_PROCESSOR_LIST "--bind-to cpulist:ordered --cpu-set 0,1,8,9,16,17,24,25,30,31,38,39,46,47,54,55,60,61,68,69,76,77,84,85,90,91,98,99,106,107,114,115 --report-bindings"

#        else if ( $SLURM_NTASKS_PER_NODE == "16" ) then

#        setenv PIN_PROCESSOR_LIST "--bind-to cpulist:ordered --cpu-set 0,8,16,24,30,38,46,54,60,68,76,84,90,98,106,114 --report-bindings"
...
#   endif
  ...
  #  ( /usr/bin/time -p mpirun -np $NPROCS $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
   (/usr/bin/time -p mpirun -np $NPROCS --bind-to l3cache --map-by ppr:24:numa -x LD_LIBRARY_PATH -x PATH -x PWD $BLD/$EXEC ) |& tee buff_${EXECUTION_ID}.txt
