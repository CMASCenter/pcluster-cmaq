...
print(csv_data)
p1 <- ggplot(csv_data, aes(y=Scaling, x=Nodes, size=SBATCHexclusive, color=COLROW)) +
    geom_point() + ggtitle("2 Day Benchmark Scaling versus Node (36cpu/node) using c5n.18xlarge") + xlim(0,8) + ylim(0,8)
    

p2 <- ggplot(csv_data, aes(y=Efficiency, x=Nodes, size=SBATCHexclusive, color=COLROW)) +
