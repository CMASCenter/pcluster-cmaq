...
png(file = paste('c5n18xlarge','_','c5n9xlarge','_','Time','_','CPUs','.png',sep=''), width = 1024, height = 768, bg='white')
csv_data<- read.csv("/shared/pcluster-cmaq/docs/user_guide_pcluster/qa/timing_both.csv",sep="\t", skip =0, header = TRUE, comment.char = "",check.names = FALSE, quote="", )
print(csv_data)
p1 <- ggplot(csv_data, aes(y=TotalTime, x=CPUs, size=InputData, color=COLROW, shape=ComputeNode)) +
    geom_point() + ggtitle("2 Day Benchmark Total Time versus CPUs") + scale_y_continuous(name = "Total Time (seconds)")

p2 <- ggplot(csv_data, aes(y=OnDemandCost, x=CPUs, size=InputData, color=COLROW, shape=ComputeNode)) +
