...
sens.dir  <- '../run_scripts/hpc6a/logs/'
base.dir  <- '../run_scripts/hpc6a/logs/'
...
# get timing info
   ...
   pdf(file = paste('hpc6a_fsx_lustre_1-6nodes_',comp,'_all',sens.name,'_',base.name,'.pdf',sep=''), width = 10, height = 7, bg='white')
   #png(file = paste('hpc6a_fsx_lustre_1-6nodes_',comp,'_all',sens.name,'_',base.name,'.png',sep=''), width = 1024, height = 768, bg='white')
  # png(file = paste(comp,'_',sens.name,'.png',sep=''), width = 1024, height = 768, bg='white')
   barplot(bar.data, main = 'Process Timing on /fsx (lustre) using 1-6 nodes with 96 cpus/node on HPC6a with pinning',names.arg = b.names,ylab='seconds', col = my.colors, legend = n.proc.plot, xlim = c(0.,xmax),ylim = c(0.,6000.))
   box()
   dev.off()
