...
if(cctm.file2==""){ 
  ...
  for(i in 1:length(plot.spec.list)){
    png(paste0(outdir,"/",plot.spec.list[i],"_",toupper(stat.fun),"_map_",appl1,".png"),height=my.height,width=my.width,units="in",res=400)
    par(mai=my.mai)
...
plot.spat.diff <- function(spec.name,spec.unit){
 ...
 plot(1,xlim=range(x.proj.coord),ylim=range(y.proj.coord),axes=F,xlab="",ylab="",type="n",main=paste("Model 1 (",appl1,"): ",spec.name,sep=""))
 axis(1,at=x.plot.seq,labels=lon.seq)	#Add longitude to x axis.
 axis(2,at=y.plot.seq,labels=lat.seq)	#Add latitude to y axis.
 ...
 plot(1,xlim=range(x.proj.coord),ylim=range(y.proj.coord),axes=F,xlab="",ylab="",type="n",main=paste("Model 2 (",appl2,"): ",spec.name,sep=""))
 axis(1,at=x.plot.seq,labels=lon.seq)	#Add longitude to x axis.
 axis(2,at=y.plot.seq,labels=lat.seq)	#Add latitude to y axis.
...
for(i in 1:length(plot.spec.list)){
 ...
 png(paste0(outdir,"/",plot.spec.list[i],"_M2M_spatial_DIFF_",toupper(stat.fun),"_",appl1,"-",appl2,".png"),height=my.height*1.5,width=my.width*1.7,units="in",res=400)
