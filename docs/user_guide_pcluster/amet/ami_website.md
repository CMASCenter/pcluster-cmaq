...
Check box or pull-down options include selection of projects, species, observation networks, and program to run. Create plots by selecting one or more observation networks and species and select a program to run using the pull-down menu. Clicking on the `Run Program` button runs the program for the selected observation network and species on the VM, queries the database, creates the plots, and provides links for the user to view either a pdf or png or html (for interactive plots) version of each plot.  
